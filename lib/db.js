...
export const db = {
  ...
  async getUserById(userId) {
    try {
      return await kv.get(`${USER_PREFIX}${userId}`);
    } catch (error) {
  ...
  async createUser(userData) {
    try {
      ...
      await kv.sadd(USERS_KEY, userId);
  ...
  async initializeUserProgress(userId) {
    try {
      ...
      await kv.set(`${PROGRESS_PREFIX}${userId}`, initialProgress);
  ...
  async getUserProgress(userId) {
    try {
      const progress = await kv.get(`${PROGRESS_PREFIX}${userId}`);
      if (!progress) {
  ...
  async saveUserProgress(userId, progressData) {
    try {
      ...
      await kv.set(`${PROGRESS_PREFIX}${userId}`, updatedProgress);
  ...
  async deleteUser(userId) {
    try {
      ...
      await kv.del(`${PROGRESS_PREFIX}${userId}`);
      await kv.srem(USERS_KEY, userId);
      return true;
    } catch (error) {
